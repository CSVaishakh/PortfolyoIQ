...
After changing any `shared/*.env` file, remember it's only *copied* into a release's `.env.local` at deploy time — editing the shared file alone doesn't affect an already-running release. Either wait for the next deploy, or apply it immediately:

```bash
sudo cp /opt/portfolioiq/shared/platform-service.env /opt/portfolioiq/current/apps/platform-service/.env.local
sudo cp /opt/portfolioiq/shared/model-service.env /opt/portfolioiq/current/apps/model-service/.env.local
sudo -n /usr/bin/systemctl restart portfolioiq-platform.service portfolioiq-model.service
```

### The admin model-training feature (`/train` page, `DEMO_MODEL_ENABLED`)

`ADMIN_SECRET` (auto-generated by bootstrap, read with `sudo grep ADMIN_SECRET /opt/portfolioiq/shared/platform-service.env`) gates the admin endpoints used by the `/train` page: `POST /model/seed`, `/model/aggregate`, `/model/train`, `/model/rollback/:serialno`. Separately, `POST /train/dataset` on `model-service` itself (which `/model/seed` calls) is additionally gated by `DEMO_MODEL_ENABLED` — the README frames this as demo/compatibility-testing, "a production-promotion decision, not a setup step," so bootstrap defaults it to `false`.

This deployment has deliberately promoted it to `true` so the `/train` page's seeding feature works in production. If you ever need to reverse that: set `DEMO_MODEL_ENABLED=false` in both `shared/*.env` files and re-apply as above. Note this flag is unrelated to `FEDERATED_CONTRIBUTIONS_ENABLED`/`FEDERATED_AGGREGATION_ENABLED` (still `false`, still paused per the README) — it only controls the demo-dataset seeding path, not user-contributed federated learning.
...
**Networking to native services**: the container runs with `network_mode: host`, not the more common bridge-network-plus-`extra_hosts` approach. The app services bind to `127.0.0.1` only (deliberately — nothing but same-host processes should reach them), and `host.docker.internal` resolves to the *bridge gateway* IP from inside a bridge-networked container, which is a different interface than loopback — a `127.0.0.1`-bound service is unreachable through it no matter what. Host networking puts Caddy directly in the host's network namespace, so `127.0.0.1:5000` inside the container is the exact same loopback the platform-service is actually listening on. (This was found the hard way: the first real deploy passed every loopback health check inside `remote-deploy.sh` but Caddy still returned 502 until this was fixed.)
